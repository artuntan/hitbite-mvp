...
## 2026-09-20 — Always-public testnet entry

- Removed the misleading Request access link and the alternate access-only landing mode. Header and main actions always open `/app`; the former `NEXT_PUBLIC_APP_LIVE` variable no longer controls entry or NAV visibility. The footer email remains a normal contact link.
- Public website access requires no invitation, login or manual approval. Existing self-service simulated wallet verification, professional-investor confirmation, US/Türkiye country restrictions and contract permissions remain in effect for transactions. No contract or verification logic changed.
- `NEXT_PUBLIC_APP_LIVE=false pnpm check` passed lint, types, all 14 configuration/security tests and the optimized build, verifying that even the legacy false setting cannot restore the access-only page. Gitleaks passed. All six landing browser groups passed on that local build, including anonymous app entry, no Request access text, four viewport sizes, navigation, NAV states, favicon and sharing metadata. No browser errors or wallet transactions occurred. Local evidence: `.context/public-entry-local-evidence.json`; server stopped. Historical generated STATUS is unchanged.

### Public entry: production verification

- PR #12 merged after all CI and Vercel preview checks passed (implementation `4e397a0`, production merge `35f132b`): https://github.com/artuntan/hitbite-mvp/pull/12. Pre-merge CI: https://github.com/artuntan/hitbite-mvp/actions/runs/35515672056. Vercel reported the production deployment complete.
- Anonymous browser verification on https://hitbite.markets passed all six landing groups: no Request access text, two direct app links, working landing-to-app navigation, four responsive viewports, NAV loading/failure states and existing accessibility/asset/metadata checks. No page errors, cookies on the landing or wallet transactions occurred. Public and separately labelled local legacy-false-build evidence: `deployments/evidence/public-entry.json`.
- Public entry is invitation-free. Transaction eligibility and country restrictions remain unchanged; no claim that every visitor is eligible to transact is made. Local server is stopped and historical generated STATUS is unchanged.


## 2026-09-20 — Public repository security and reviewability

- Enabled GitHub secret scanning and push protection, dependency alerts/security updates, CodeQL default analysis and private vulnerability reporting. The first CodeQL scan completed successfully with no open alerts; the production dependency audit reported zero known advisories across 535 dependencies at this time. The prior bounded history/log/artifact exposure review is documented with its limits, without publishing raw findings or credentials.
- Enabled protected main: a PR, the four existing CI jobs, an up-to-date branch and resolved conversations are required, with administrator enforcement and no force pushes/deletion. Zero approving reviews reflects the single-maintainer setup. External fork workflows now require approval; default token permissions remain read-only.
- Pinned workflow Actions to full commit SHAs and added weekly Dependabot updates, CODEOWNERS, contribution/security policies, issue forms and a PR template. Reworked README around source, live app, verified contracts and dated evidence; moved operational/model details to focused documentation. MIT and generated historical STATUS are unchanged.
- Replaced daily NAV's direct main push with a two-file snapshot PR, an explicit CI dispatch, exact-commit success validation and a normal protected merge. No PAT or protection bypass is introduced. Role secrets are scoped to the publishing steps. Live automation verification follows after merge.
- Published an edge firewall rule for POST /api/verify: 20 requests per IP per 60 seconds. A bounded live test returned 429 on request 21 while the landing remained 200; no signatures or transactions were sent. Removed registrar and ticket secrets from future Preview deployments while retaining Production values and existing fork protection.
- Added streaming 8-KiB byte limits, malformed-envelope validation before RPC, friendly HTML 429/503 handling, and frame/content-type/referrer/permissions headers. Local workspace checks passed 19 configuration/security tests and the optimized build; NAV math passed 10 tests and deterministic dry-run, publisher guards passed two tests, and a real gitleaks regression caught a secret introduced then deleted in an intermediate commit without printing it. Candidate gitleaks and browser checks are recorded separately; CI/production verification follows.

### Protected publication integration correction

- Source PR #13 passed required CI, CodeQL and Vercel checks and merged as `cfa9177`. The first protected NAV integration run created PR #14 and passed explicitly dispatched CI, but GitHub kept the PR's separately generated workflow in approval-required state and refused the merge. Protection correctly failed closed; no bypass was used.
- Updated the publisher to start the actual PR check suite, approving only the current job's generated snapshot PR after matching its repository, head, branch, bot author and two-file scope. It waits for those exact checks and a clean merge state. External fork approval remains mandatory. Added a negative test for foreign heads, branches, repositories, actors, events and PR numbers. The failed integration run is retained as evidence rather than presented as a pass.
- Live browser checks passed readable HTML 429/503 fixtures, malformed/oversized API rejection, cross-origin rejection and security headers. A separate fresh-wallet browser run completed real on-chain verification and seven shell/form checks with no browser errors. Its test funding and registrar receipts are recorded separately from the transaction-free error fixtures.

- The corrected workflow's first two attempts stopped at the primary Arc RPC with an HTTP error before reaching PR automation. The oracle remained funded (approximately 0.49 test USDC); a local read-only transaction build/gas estimate passed. Added an explicit `NAV_RPC_URL` repository variable and selected Arc's documented dRPC testnet endpoint for this scheduled job. Chain-ID validation stays in place; browser and registrar RPC settings are unchanged. The failed attempts remain in the run history.

- Full snapshot diagnostics identified dRPC rejecting a bounded 2,000-block historical `eth_getLogs` request with HTTP 400. QuickNode's Arc-documented endpoint passed the complete read-only snapshot (including all 24 historical log ranges), so `NAV_RPC_URL` now explicitly uses `https://rpc.quicknode.testnet.arc.io`. No NAV math, role or contract change was needed.

- The complete local publisher successfully confirmed a diagnostic NAV update through QuickNode (`0x0b334fa8cd3e070a78e02bdb155bbf9ab09ccce2ccfd609a5f2a1427313c14ca`), while the hosted job still failed in its publication stage. Added a public signer-role preflight and safe failure diagnostics containing only exception type, engine source location and numeric HTTP status; provider messages, URLs, request bodies and signing data remain suppressed. A regression test checks this redaction boundary. This diagnostic write is separate from the read-only API/browser checks above.

- Confirmed NAV transactions are now logged before post-receipt snapshot reads. Those reads retry the same pinned block on transient HTTP/transport/indexing failures, with bounded backoff; retries never sign or submit another transaction. Invalid model/configuration errors still fail immediately. Tests cover eventual read success, identical block pinning, bounded failure and no retry for a model error. This handles receipt/state propagation differences without masking a failed publication or silently retrying a write.

### Public repository hardening: completed production verification

- Source/security PR #13, protected-workflow correction #16, RPC configuration #17 and confirmed-read diagnostics/retry #18 all merged after required CI checks. Workflow Actions are SHA-pinned and repository policy enforces full-SHA pinning. CodeQL and the production dependency scan reported no open/known findings at the recorded check; this remains an internal review rather than an independent audit.
- Protected NAV run https://github.com/artuntan/hitbite-mvp/actions/runs/35519987522 completed successfully. It created and approved only its own generated PR #19, ran the actual required PR suite, merged normally under administrator-enforced main protection as `3594a9a`, and verified that Vercel served the new publication. No PAT, direct-main push or protection bypass was used. Earlier failed runs are retained and linked in the dated evidence.
- The final read-only live smoke check passed all seven groups: chain, code, settlement decimals, fresh matching NAV, attestor signature, country blocklist and all product routes. The successful snapshot is timestamped 2026-09-20T15:34:29Z.
- Public evidence in `deployments/evidence/public-repo-hardening.json` records actual GitHub settings, bounded exposure-review scope, the zero-advisory production dependency result, live firewall 429 test, local/live API and browser checks, fresh-wallet funding/verification receipts, and successful protected publication. Error fixtures sent no transactions; actual registrar/funding and diagnostic NAV writes are separately identified. Local browser/server processes are stopped, private values are not published, and generated historical STATUS and MIT licensing remain unchanged.
