...
export function parseChainName(value: string | undefined): ChainName {
  ...
  throw new Error(
    "NEXT_PUBLIC_CHAIN must be arc-testnet, base-sepolia, or local.",
  );
}
...
export function assertChainId(actualId: number, selected: ChainName): void {
  if (
    !allowedChainIds.some((id) => id === actualId) ||
    chains[selected].id !== actualId
  ) {
    throw new Error(
      "RPC chain ID does not match the selected allowlisted test network.",
    );
  }
}
