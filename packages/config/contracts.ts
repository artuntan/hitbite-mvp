...
export function getDeployment(name: ChainName): Deployment | null {
  ...
  if (
    ...
    item.chainId !== chains[name].id ||
    !Number.isSafeInteger(item.blockNumber) ||
    item.blockNumber < 0
  )
    throw new Error("Deployment metadata does not match the selected chain.");
  for (const address of Object.values(item.addresses)) {
    if (!isAddress(address) || /^0x0{40}$/i.test(address))
      throw new Error("Deployment contains an invalid address.");
  }
  if (
    name === "arc-testnet" &&
    item.addresses.USDC.toLowerCase() !== ARC_USDC_ADDRESS.toLowerCase()
  )
    throw new Error("Arc must use its native USDC ERC-20 interface.");
  return item;
}
