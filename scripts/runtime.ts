...
import { assertChainId, chains, parseChainName, ARC_MIN_MAX_FEE_PER_GAS } from "../packages/config/chains.ts";
...
export async function context(selected?: string) {
  ...
  const rpcUrl = process.env.RPC_URL || process.env.NEXT_PUBLIC_RPC_URL || chain.rpcUrls.default.http[0];
...
export function accountFor(variable: string) {
  ...
  if (!value || !/^0x[0-9a-fA-F]{64}$/.test(value)) throw new Error(`${variable} is missing or invalid.`);
  try { return privateKeyToAccount(value as Hex); } catch { throw new Error(`${variable} is invalid.`); }
}
...
export async function fees(ctx: Context) {
  ...
  return { maxFeePerGas: estimate.maxFeePerGas > floor ? estimate.maxFeePerGas : floor, maxPriorityFeePerGas: estimate.maxPriorityFeePerGas };
}

export async function transact(ctx: Context, variable: string, address: Address, abi: Abi, functionName: string, args: readonly unknown[] = []) {
  assertChainId(await ctx.client.getChainId(), ctx.name);
  const account = accountFor(variable);
  const data = encodeFunctionData({ abi, functionName, args });
  const fee = await fees(ctx);
  const gas = await ctx.client.estimateGas({ account, to: address, data, ...fee });
  const wallet = createWalletClient({ account, chain: ctx.chain, transport: http(ctx.rpcUrl) });
  const hash = await wallet.sendTransaction({ to: address, data, gas: gas * 12n / 10n, ...fee });
...
export function readDeployment(name: string) {
  ...
  if (data.status !== "confirmed") throw new Error("Deployment has no confirmed receipts.");
  return data as {
    status: "confirmed"; chainId: number; blockNumber: number;
    addresses: { IdentityRegistry: Address; HBToken: Address; USDC: Address };
    deployer: Address; timestamp: string; transactions: Record<string, Hex>;
    roles: { admin: Address; issuer: Address; oracle: Address; registrar: Address };
    vaultFunding: { amount: string; transactionHash: Hex; blockNumber: number }[];
...
export function safeError(error: unknown) {
  ...
  for (const [name, value] of Object.entries(process.env)) {
    if (value && value.length > 6 && /PRIVATE_KEY|TOKEN|SECRET|AUTH|RPC_URL/.test(name)) message = message.replaceAll(value, "[redacted]");
  }
  return message;
}
