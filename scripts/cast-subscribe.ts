...
const wallet = createWalletClient({
  account,
  chain: ctx.chain as Chain,
  transport: http(ctx.rpcUrl),
});
const verified = await ctx.client.readContract({
...
for (const [address, signature, args] of [
  [
    ...
    [d.addresses.HBToken, "500000"],
  ],
  [d.addresses.HBToken, "subscribe(uint256)", ["500000"]],
] as const) {
  const encoded = spawnSync("cast", ["calldata", signature, ...args], {
    encoding: "utf8",
  });
  ...
  const sent = spawnSync(
    "cast",
    ["publish", "--async", "--rpc-url", ctx.rpcUrl, signed],
    { encoding: "utf8" },
  );
  ...
  if (receipt.status !== "success")
    throw new Error("cast-published transaction reverted");
  records.push({
    step: signature,
