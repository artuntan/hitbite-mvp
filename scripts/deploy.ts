...
try {
  const { values } = parseArgs({
    options: { chain: { type: "string" }, "dry-run": { type: "boolean" } },
  });
  const ctx = await context(values.chain);
  const target = `deployments/${ctx.name}.json`;
  if (existsSync(target) && !values["dry-run"])
    throw new Error(
      "Deployment already exists. Review it before deploying new contracts.",
    );
  ...
  writeFileSync(
    ".context/deploy.log",
    safeError((result.stdout || "") + (result.stderr || "")),
  );
  if (result.status !== 0)
    throw new Error(
      "Foundry deployment failed. See redacted .context/deploy.log.",
    );
  if (values["dry-run"]) {
    console.log(
      `Deployment simulation passed on ${ctx.name}; no transactions sent.`,
    );
  } else {
    const broadcast = JSON.parse(
      readFileSync(
        `contracts/broadcast/Deploy.s.sol/${ctx.chain.id}/run-latest.json`,
        "utf8",
      ),
    );
    ...
    for (const tx of broadcast.transactions) {
      ...
      const code = await ctx.client.getCode({
    ...
    if (!addresses.HBToken || !addresses.IdentityRegistry || !addresses.USDC)
      throw new Error("Incomplete deployment receipts.");
    ...
    writeFileSync(
      ...
      json({
        ...
        transactions,
        vaultFunding: [],
        explorer: ctx.chain.blockExplorers?.default.url ?? null,
      }),
    );
