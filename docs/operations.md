...
`NAV_RPC_URL` is an optional repository variable for the publisher's public RPC endpoint. It currently selects `https://rpc.drpc.testnet.arc.io`, an [Arc-documented testnet provider](https://docs.arc.io/arc/references/rpc-endpoints), after repeated HTTP failures from the primary endpoint during publication. The engine still validates chain ID 5042002. Keep credential-bearing RPC URLs in a secret store instead of this public variable. This does not alter the browser or registrar RPC configuration.
