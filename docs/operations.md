...
`NAV_RPC_URL` is an optional repository variable for the publisher's public RPC endpoint. It currently selects `https://rpc.quicknode.testnet.arc.io`, an [Arc-documented testnet provider](https://docs.arc.io/arc/references/rpc-endpoints), after HTTP failures from the primary endpoint during publication and rejected historical log queries at dRPC. A full read-only NAV snapshot was verified against this endpoint before selecting it. The engine still validates chain ID 5042002. Keep credential-bearing RPC URLs in a secret store instead of this public variable. This does not alter the browser or registrar RPC configuration.

The publisher allows only `app/public/data/nav.json` and `app/public/data/attestation.json` to change. It opens an isolated branch/PR, starts its own PR CI, verifies a fresh successful run for that exact commit, then requests a normal protected merge. The publisher approves only its own generated snapshot PR workflow after checking the exact head, repository, author and file scope. External PRs remain subject to maintainer approval. This handles [events created by GITHUB_TOKEN have special trigger rules](https://docs.github.com/en/actions/how-tos/write-workflows/choose-when-workflows-run/trigger-a-workflow#triggering-a-workflow-from-a-workflow).
