...
- **D14 MockUSDC faucet.** `faucet(to, amount)`: `amount ≤ 10_000e6` per call and `≤ 10_000e6` per `to` per rolling 24 h (`lastFaucetDay`/`faucetedToday` maps). Name `MockUSDC (Testnet)`, symbol `mUSDC`.
...
- **D20 `supplyBackedRatio()`.** On-chain: `(usdc.balanceOf(this) + reportedAUM) × 1e18 / (totalSupply × nav / 1e18)`; `1e18` when supply is 0. Labelled illustrative: on testnet subscription USDC sits in the vault while the portfolio is simulated.
