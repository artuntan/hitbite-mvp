...
## 2026-09-20 — Founder UX revision

The founder replaces the overview and original navigation/three-column app layout: `/` opens the investor flow, `/app` remains a compatible entry point, and the header contains only the brand, Transparency and a position summary. The header follows scrolling with a translucent glass surface. Investor screens use a single focused card, a compact step navigator, one primary action, and progressive disclosure for explanatory/technical details. Position moves into a header popover; walkthrough help moves out of the navigation. Contract behavior, eligibility checks and receipt evidence remain intact. This explicit feedback supersedes conflicting route/layout instructions in the original design source. Historical E2E STATUS remains unchanged; UI regression evidence for this revision is recorded separately.

## 2026-09-20 — Founder refinement: progressive setup and account workspace

The founder keeps the current visual direction and requests a transparent header at the top of the page, with glass appearing smoothly as the page scrolls. The completed setup steps close automatically: connect → eligibility → first subscription. A confirmed investment opens a wider, technical account workspace with real balances, position/NAV/liquidity data, subscription/redemption orders, coupon payout and wallet transaction history. The workspace preference is scoped to wallet, chain and contract and survives a full redemption in the same browser; on-chain balances independently open it for existing holders. Eligibility remains enforced by the registry and contract. All displayed data remains actual testnet data, with simulated-fund disclosures retained. This replaces the earlier clickable five-step investor navigation and separate completion/hold screens. Historical STATUS remains unchanged; browser and live transaction evidence is recorded separately.
