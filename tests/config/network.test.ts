...
test("fallback changes configuration using only the selected environment value", () => {
  assert.equal(
    readPublicConfig({ NEXT_PUBLIC_CHAIN: "base-sepolia" }).chain.id,
    84532,
  );
  assert.equal(
    readPublicConfig({ NEXT_PUBLIC_CHAIN: "local" }).rpcUrl,
    "http://127.0.0.1:8545/",
  );
...
test("unknown, production and prototype-property chain selections fail closed", () => {
  for (const value of [
    ...
    assert.throws(
      () => readPublicConfig({ NEXT_PUBLIC_CHAIN: value }),
      /NEXT_PUBLIC_CHAIN/,
    );
  }
...
test("RPC overrides retain the explicit selected-chain check", () => {
  const config = readPublicConfig({
    NEXT_PUBLIC_RPC_URL: "https://rpc.drpc.testnet.arc.io",
  });
...
test("invalid public URLs reject without echoing embedded credentials", () => {
  ...
  for (const value of [
    ...
    `ftp://example.com/${marker}`,
    marker,
    "http://remote.example.com",
  ]) {
    assert.throws(
      () => readPublicConfig({ NEXT_PUBLIC_RPC_URL: value }),
      (error: unknown) => {
  ...
  assert.throws(() =>
    readPublicConfig({ NEXT_PUBLIC_RPC_URL: "http://localhost:8545" }),
  );
  assert.doesNotThrow(() =>
    readPublicConfig({
...
test("public config never returns unrelated private environment fields", () => {
  const env = {
    ...
    ORACLE_PRIVATE_KEY: "do-not-copy",
  };
...
test("attestation signer accepts a public address and rejects zero or malformed addresses", () => {
  assert.doesNotThrow(() =>
    readPublicConfig({ NEXT_PUBLIC_ATTESTOR_ADDRESS: ARC_USDC_ADDRESS }),
  );
  for (const address of ["invalid", `0x${"0".repeat(40)}`]) {
    assert.throws(
      () => readPublicConfig({ NEXT_PUBLIC_ATTESTOR_ADDRESS: address }),
      /ATTESTOR_ADDRESS/,
    );
  }
});
